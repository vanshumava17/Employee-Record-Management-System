...
recordCountString = randomAccessFile.readLine().trim();         // here trim() is imp bcoz data stored in file as "0          " so to trim those spaces
...
if(file.exists() == false)throw new DAOException("Invalid Code "+ code);
RandomAccessFile randomAccessFile = new RandomAccessFile(file,"rw");
if(randomAccessFile.length() == 0)throw new DAOException("Invalid Code "+code);
randomAccessFile.readLine();
randomAccessFile.readLine();
int fcode;
String fTitle;
...
RandomAccessFile randomAccessFile = new RandomAccessFile(file,"rw");
if(randomAccessFile.length() == 0)throw new DAOException("Invalid title "+title);
randomAccessFile.readLine();
randomAccessFile.readLine();
int fcode;
String fTitle;
...
if(code > lastGeneratedCode)return false;
randomAccessFile.readLine();
int fcode;
String fTitle;
while(randomAccessFile.getFilePointer() < randomAccessFile.length())
